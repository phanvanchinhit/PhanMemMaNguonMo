...
class Product extends MY_Controller
{
    ...
    function add()
    {
        ...
        if ($this->input->post()) {
            $this->form_validation->set_rules('name', 'bắt buộc nhập', 'required');
            $this->form_validation->set_rules('catalog', 'bắt buộc nhập', 'required');
            if ($this->form_validation->run()) {
                ...
                if ($this->product_model->create($data)) {
                    // neu them thanh cong
                    $this->session->set_flashdata('message', 'Thêm mới thành công môn học');
    ...
    function edit()
    {
        ...
        if (!$product_info) {
            ...
            $this->session->set_flashdata('message', 'Không tồn tại môn học này');
            redirect(admin_url('product'));
        }
        $this->load->model('catalog_model');
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->product_model->update($product_info->id_product, $data)) {
                    ...
                    // in ra thong bao loi
                    $message = $this->session->set_flashdata('message', 'Có lỗi khi sửa');
                    redirect(admin_url('product'));
                }
            }
        }
